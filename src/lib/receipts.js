...
export async function submitReceipt({ userId, totalAmount, images, allocations, hasImage, depositDateJalali, cardLast4 }) {
  const { data: receipt, error } = await supabase
    ...
    .insert({
      ...
      // For "no receipt image", the image is only considered present
      // after the Edge Function has generated, uploaded, and linked it.
      has_image: hasImage,
  ...
  if (hasImage) {
    ...
    // without a bank receipt. The Edge Function sets has_image=true only
    // after the Storage upload and receipt_images link both succeed.
    const { error: imageError } = await supabase.functions.invoke('generate-no-receipt-image', {
      body: { receipt_id: receipt.id },
    })
    if (imageError) {
      return { data: receipt, error: imageError }
    }
  }
...
export async function listReceiptsByUser(userId) {
  const { data, error } = await supabase
    .from('receipts')
    .select(
      '*, receipt_images(storage_path), receipt_installments(amount_applied, installments(due_amount, paid_amount, fund_months(jalali_month), shares(fund_codes(code_number, funds(name)))))'
    )
    .eq('user_id', userId)
    .order('created_at', { ascending: false })
  return { data, error }
...
export async function finalConfirmReceipt(receiptId) {
  const { data: receipt } = await supabase
    .from('receipts')
    .select(
      'id, deposit_date_jalali, receipt_installments(installment_id, amount_applied, installments(due_amount, paid_amount, fund_months(jalali_month)))'
    )
    .eq('id', receiptId)
    .single()
  ...
  // Goodwill: positive if paid before the 15th of the installment's own
  // due month, negative if after — measured as real calendar days, so
  // paying a month (or more) early is correctly a big positive score
  // instead of being misread as "day 29 is after day 15".
  const depositOrdinal = receipt?.deposit_date_jalali ? jalaliOrdinal(receipt.deposit_date_jalali) : null
  ...
  for (const ri of receipt?.receipt_installments ?? []) {
    ...
    const dueMonth = ri.installments.fund_months?.jalali_month
    if (depositOrdinal !== null && dueMonth) {
      const dueOrdinal = jalaliOrdinal(`${dueMonth}-15`)
      goodwillScore = dueOrdinal - depositOrdinal
    }

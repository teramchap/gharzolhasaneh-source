...
function daysInJalaliMonthNumber(month) {
  if (month <= 6) return 31
  if (month <= 11) return 30
  return 29
}

// Converts a "YYYY-MM-DD" Jalali date into a linear day count, so two
// dates can be subtracted to get the number of days between them —
// correctly handling month/year boundaries (e.g. paying a month early).
export function jalaliOrdinal(dateStr) {
  const [y, m, d] = dateStr.split('-').map(Number)
  let days = 0
  for (let mm = 1; mm < m; mm++) days += daysInJalaliMonthNumber(mm)
  return y * 365 + days + d
}
